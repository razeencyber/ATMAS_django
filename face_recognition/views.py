...
def detectFace(request):
    ...
    def get_blink_ratio(eye_points, facial_landmarks):
        ...
        mid_top = mid_point(facial_landmarks.part(eye_points[1]),
                            facial_landmarks.part(eye_points[2]))
        mid_bottom = mid_point(facial_landmarks.part(eye_points[5]),
                               facial_landmarks.part(eye_points[4]))

        hor_dist = cv2.line(img, left_point, right_point, (255, 0, 0), 2)
        ver_dist = cv2.line(img, mid_top, mid_bottom, (255, 0, 0), 2)

        hor_dist_length = hypot((left_point[0] - right_point[0]),
                                (left_point[1] - right_point[1]))
        ver_dist_length = hypot((mid_top[0] - mid_bottom[0]),
    ...
    while (True):
        ...
        for face in faces:
            ...
            left_eye_ratio = get_blink_ratio([36, 37, 38, 39, 40, 41],
                                             landmarks)
            right_eye_ratio = get_blink_ratio([42, 43, 44, 45, 46, 47],
                                              landmarks)
            ...
            print(
                f"getId {getId} card_number {card_number} conf {conf} blinking_ratio {avg_blink_ratio}"
            )
            ...
            if conf < 65 and getId == int(card_number):
                if avg_blink_ratio > 8.0:
                    ...
                    cv2.putText(img, "Fake", (x, y1), font, 2, (0, 0, 255), 2)
                    cv2.putText(img, "Please Blink", (20, 150), font, 2,
                                (255, 0, 0), 3)
